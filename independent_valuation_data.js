...
const INDEPENDENT_VALUATION_DATA = {
  ...
  "AMAT": {
    "relativeValueBand": {
      ...
      "generatedAt": "2026-09-23T10:40:58.413435",
      "asOfDate": "2026-09-23",
      "lastCheckStatus": "SKIPPED_RATE_LIMIT",
      "lastCheckNote": "직전 기록(2026-09-23T10:40:58.413435) 이후 38.0초밖에 지나지 않아(60초 미만) 이번 실행은 건너뜀.",
      "lastCheckWasBenignSkip": true,
      "lastCheckBenignSkipNote": "마지막 실행(SKIPPED_RATE_LIMIT)은 직전 판단과 너무 가까운 시간에 다시 요청돼 중복 호출로 생략됐습니다(사유: 직전 기록(2026-09-23T10:40:58.413435) 이후 38.0초밖에 지나지 않아(60초 미만) 이번 실행은 건너뜀.) -- 재계산하지 않고 아래 마지막 정상 스냅샷을 그대로 재사용합니다. 이 스냅샷 자체는 아래 노후 기준으로도 여전히 유효합니다.",
  ...
  "LRCX": {
    "relativeValueBand": {
      ...
      "changeReasonVsPrevious": "입력값 완전 동일(변경 없음) -- 밴드(EPS·peer 배수) 근거 불변, 현재가만 바뀌었어도 밴드 자체는 재계산하지 않음",
      "generatedAt": "2026-09-23T10:40:58.656932",
      "asOfDate": "2026-09-23",
      "lastCheckStatus": "SKIPPED_RATE_LIMIT",
      "lastCheckNote": "직전 기록(2026-09-23T10:40:58.656932) 이후 37.8초밖에 지나지 않아(60초 미만) 이번 실행은 건너뜀.",
      "lastCheckWasBenignSkip": true,
      "lastCheckBenignSkipNote": "마지막 실행(SKIPPED_RATE_LIMIT)은 직전 판단과 너무 가까운 시간에 다시 요청돼 중복 호출로 생략됐습니다(사유: 직전 기록(2026-09-23T10:40:58.656932) 이후 37.8초밖에 지나지 않아(60초 미만) 이번 실행은 건너뜀.) -- 재계산하지 않고 아래 마지막 정상 스냅샷을 그대로 재사용합니다. 이 스냅샷 자체는 아래 노후 기준으로도 여전히 유효합니다.",
      "priceAgeDaysAtBuild": 1,
      "snapshotAgeDaysAtBuild": 0,
      "priceDateClass": "ok",
      "snapshotDateClass": "ok",
  ...
  "KLAC": {
    "relativeValueBand": {
      ...
      "changeReasonVsPrevious": "입력값 완전 동일(변경 없음) -- 밴드(EPS·peer 배수) 근거 불변, 현재가만 바뀌었어도 밴드 자체는 재계산하지 않음",
      "generatedAt": "2026-09-23T10:40:58.855887",
      "asOfDate": "2026-09-23",
      "lastCheckStatus": "SKIPPED_RATE_LIMIT",
      "lastCheckNote": "직전 기록(2026-09-23T10:40:58.855887) 이후 37.7초밖에 지나지 않아(60초 미만) 이번 실행은 건너뜀.",
      "lastCheckWasBenignSkip": true,
      "lastCheckBenignSkipNote": "마지막 실행(SKIPPED_RATE_LIMIT)은 직전 판단과 너무 가까운 시간에 다시 요청돼 중복 호출로 생략됐습니다(사유: 직전 기록(2026-09-23T10:40:58.855887) 이후 37.7초밖에 지나지 않아(60초 미만) 이번 실행은 건너뜀.) -- 재계산하지 않고 아래 마지막 정상 스냅샷을 그대로 재사용합니다. 이 스냅샷 자체는 아래 노후 기준으로도 여전히 유효합니다.",
      "priceAgeDaysAtBuild": 1,
      "snapshotAgeDaysAtBuild": 0,
      "priceDateClass": "ok",
      "snapshotDateClass": "ok",
  ...
  "ASML": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-23T10:41:36.549925",
      "neverSucceeded": true
    },
    "longTermScenario": {
  ...
  "STX": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-23T10:41:36.733133",
  ...
  "WDC": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-23T10:41:36.823088",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
  ...
  "SNDK": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-23T10:41:36.921640",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
  ...
  "PWR": {
    "relativeValueBand": {
      ...
      "withheldStatus": "JUDGMENT_WITHHELD",
      "withheldReason": "선정 기준을 통과한 동종업계 배수가 하나도 없음 -- 밴드 산출 보류(0으로 채우지 않음)",
      "withheldCheckedAt": "2026-09-23T10:41:36.964717",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
  ...
  "EME": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-23T10:41:37.006762",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
  ...
  "FIX": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-23T10:41:37.092304",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
