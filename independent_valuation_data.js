...
const INDEPENDENT_VALUATION_DATA = {
  ...
  "AMAT": {
    "relativeValueBand": {
      ...
      "generatedAt": "2026-09-21T14:16:51.560473",
      "asOfDate": "2026-09-21",
      "lastCheckStatus": "OK",
      "lastCheckNote": null,
  ...
  "LRCX": {
    "relativeValueBand": {
      ...
      "changeReasonVsPrevious": "입력값 완전 동일(변경 없음) -- 밴드(EPS·peer 배수) 근거 불변, 현재가만 바뀌었어도 밴드 자체는 재계산하지 않음",
      "generatedAt": "2026-09-21T14:16:51.752340",
      "asOfDate": "2026-09-21",
      "lastCheckStatus": "OK",
      "lastCheckNote": null,
      "priceAgeDaysAtBuild": 3,
  ...
  "KLAC": {
    "relativeValueBand": {
      ...
      "changeReasonVsPrevious": "입력값 완전 동일(변경 없음) -- 밴드(EPS·peer 배수) 근거 불변, 현재가만 바뀌었어도 밴드 자체는 재계산하지 않음",
      "generatedAt": "2026-09-21T14:16:51.934614",
      "asOfDate": "2026-09-21",
      "lastCheckStatus": "OK",
      "lastCheckNote": null,
      "priceAgeDaysAtBuild": 3,
  ...
  "ASML": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-21T14:16:52.119258",
      "neverSucceeded": true
    }
  },
  "STX": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-21T14:16:52.283761",
  ...
  "WDC": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-21T14:16:52.368754",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
  ...
  "SNDK": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-21T14:16:52.452477",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
  ...
  "PWR": {
    "relativeValueBand": {
      ...
      "withheldStatus": "JUDGMENT_WITHHELD",
      "withheldReason": "선정 기준을 통과한 동종업계 배수가 하나도 없음 -- 밴드 산출 보류(0으로 채우지 않음)",
      "withheldCheckedAt": "2026-09-21T14:16:52.498151",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
  ...
  "EME": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-21T14:16:52.542425",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
  ...
  "FIX": {
    "relativeValueBand": {
      ...
      "withheldCheckedAt": "2026-09-21T14:16:52.628363",
      "neverSucceeded": false,
      "priorLocalSnapshotFound": true,
      "priorLocalSnapshotNote": "로컬에 이전 산출물(스냅샷)이 있었고, peer 적합성/입력 정의 재검토 결과 무효화되어 지금은 보류 상태임. 이 필드는 로컬 산출 이력만 나타내며, 그 값이 실제로 공개 URL에 노출된 적이 있었는지는 이 함수가 확인할 수 없다 -- 공개 이력은 별도로 확보한 증거(공개 URL 캡처 등)로만 판단해야 한다. 아래 prior* 필드는 로컬 역사적 기록일 뿐 현재 유효한 참고범위가 아니다.",
